...
A report definition is currently stored in Oracle database (table RC_SET ++) and is edited by the SmP Calculator tool. The definition format is basically the same as for SmP time series reports that was introduced in the beginning of 90's. A report definition consists of the following sections:
...
value attributes on those, as well as attributes that describe how object types relate to other object types. A Mesh model consists
...
*may* expand to multiple time series, whereas the SmP version provides a 1:1 ratio between column definition and
...
also depends on the power and flexibility of the *order by* feature. How to utilize *order by* to achieve a desired layout is discussed [in this document](./sort_contents.md).
...
with Mesh model instances that hold time series. To describe where to look for time series we use the Mesh model *definition*.
The TS_BINDING value is a query and not a fixed reference, therefore a template report will adapt to the current state
of the Mesh model.

A query against a given Mesh model may result in zero, one or *many* time series. This dynamic behavior also has
impact on how other report attributes are defined, for instance by using macros that make the
...
- By user in a Nimbus ad hoc report task
...
The figure above summarizes the report activation process and the players involved. Some comments:
...
- The report definition contains one or more report column definitions, each having a TS_BINDING attribute that targets time series attributes in the Mesh model
  - At this stage the designer makes a choice whether this is a report definition that assumes the start point is a specific Mesh object type or not
  - This choice impacts on how the TS_BINDING specifications are formulated.
...
- The user has the freedom to select starting point(s) and time period from where the time series data is requested when in Ad Hoc report context.
...
The value of a TS_BINDING attribute is a search specification that based on a starting point finds time series attributes in a Mesh model.
...
- there is none of the above data sources, but the `Session series` feature is activated. In this case there will be created
