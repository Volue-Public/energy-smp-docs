...
- [Mesh Service Installation Guide](#mesh-service-installation-guide)
    ...
    - [Configuration](#configuration)
        ...
        - [Limit time series cache usage](#limit-time-series-cache-usage)
            - [Monitor and tune the cache](#monitor-and-tune-the-cache)
...
Note that port 50051 in the example above belongs to the default [dynamic port range](https://en.wikipedia.org/wiki/Ephemeral_port). This means it can be occupied by another service when Mesh is starting up. In this case, Mesh displays an error message and terminates.

Configuring the gRPC server to use a port outside of the dynamic port range can minimize the risk of conflicts.
You should always check if the chosen port number is not conflicting with another service running on the system.
...
2. `stalls` should not happen in normal usage. A two digit increase in stalls
   ...
   be reported to Volue support as they may indicate a current or future
   performance issue.
...
if the cache has space for 100 entries but there is a repeating workload
accessing 101 entries in a sequence, the cache will always evict the next
item to be accessed.

This is why it is important to monitor `evictedSeries` and `misses` and to
...
| Mesh server / client | v1.11.0 | v1.12.0 | v1.13.0 | v1.14.0 | v1.15.0 | v1.16.0 |
|----------------------|---------|---------|---------|---------|---------|---------|
| **2.17 / 2025.1** | ❌ | ✔️ | ❌ | ❌ | ❌ | ❌ |
| **2.18 / 2025.2** | ❌ | ✔️ | ✔️ | ✔️ | ❌ | ❌ |
| **2.19 / 2025.3** | ❌ | ✔️ | ✔️ | ✔️ | ❌ | ❌ |
