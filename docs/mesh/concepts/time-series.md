...
The primary data retrieved from the Mesh server are time series. A time series
...
The resolution defines the time step of the time series points. Some examples
are hourly or daily resolutions. A special type of resolution is **breakpoint**.
It means there is no time step restriction and points can be provided for any
timestamps.
...
via a [time series attribute](./modelling/general.md#attribute). Either
directly or as part of a calculation.
...
calculation expressions whereas virtual time series may only use other virtual
or physical time series from the *resources* in calculation expressions. As
a result, calculation time series may also use other attribute types in
expressions, like an attribute of type double.
