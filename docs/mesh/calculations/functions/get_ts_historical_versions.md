...
Returns an array of time series versions of a time series for requested
time period. The number of versions returned can be limited to a given number.
...
Observe that function also in this case returns an array of series that in
this case contains one series.
...
Consider using the function [GetTsAsOfTime](./get_ts_as_of_time.md) in stead of using this function to 
get an array of time series versions and then retrieve one of them by using the function [AT](./at.md).
