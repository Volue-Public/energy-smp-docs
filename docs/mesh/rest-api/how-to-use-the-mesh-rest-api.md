...
User changes that are added to a session are not shared outside of the session before it is committed to the database using the `Commit` method. This method will save all changes added since the last `Commit` or `Rollback` operation.
...
This section describes how to modify requests in order to handle requests when the Mesh gRPC interface is configured to authorise the request based on the user making these requests.
...
- The authorisation method and a space character (e.g. "Basic ") is then prepended to the encoded string.
